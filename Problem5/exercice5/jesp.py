...
class JESP(Node):
    def __init__(self):
        super().__init__('jesp')
        self.get_logger().info("Initializing JESP algorithm")
        ...
        self.get_logger().info("JESP node initialized")
    ...
    def discretize_state_space(self):
        ...
        if not self.destination or len(self.robot_positions) < self.num_robots:
            ...
            # Utiliser une grille adaptative avec une résolution plus fine près des robots et de la destination
    ...
    def calculate_policies(self):
        """Calculer les politiques des robots avec l'algorithme JESP"""
        if not self.destination or len(self.robot_positions) < self.num_robots:
            # Destination ou positions des robots non encore définies
            return

        self.get_logger().info("Calculating policies using JESP algorithm")
        ...
        # Exécuter l'algorithme JESP (Joint Equilibrium-based Search for Policies)
        converged = False
        iteration = 0
        max_jesp_iterations = 10  # Limiter le nombre d'itérations de JESP

        while not converged and iteration < max_jesp_iterations:
            self.get_logger().info(f"JESP global iteration {iteration+1}/{max_jesp_iterations}")
            policies_changed = False

            # Pour chaque robot, améliorer sa politique en gardant les autres fixes
            for robot_id in range(1, self.num_robots + 1):
                robot_start_time = time.time()
                self.get_logger().info(f"Optimizing policy for robot {robot_id}")

                # Obtenir les politiques des autres robots
                other_policies = {r: p for r, p in self.robot_policies.items() if r != robot_id}

                # Calculer la nouvelle politique pour ce robot (meilleure réponse)
                new_policy = self.calculate_best_response(robot_id, other_policies)
                ...
                if self.policy_has_changed(self.robot_policies[robot_id], new_policy):
                    ...
                    robot_elapsed = time.time() - robot_start_time
                    self.get_logger().info(f"Policy for robot {robot_id} updated in {robot_elapsed:.2f}s")
                else:
                    robot_elapsed = time.time() - robot_start_time
                    self.get_logger().info(f"No change in policy for robot {robot_id} ({robot_elapsed:.2f}s)")

            # Si aucune politique n'a changé, on a convergé
            if not policies_changed:
                converged = True
                total_elapsed = time.time() - start_time
                self.get_logger().info(f"JESP converged after {iteration+1} iterations in {total_elapsed:.2f}s")
            else:
                iteration_elapsed = time.time() - start_time
                self.get_logger().info(f"JESP iteration {iteration+1} completed in {iteration_elapsed:.2f}s")
    ...
    def policy_has_changed(self, old_policy, new_policy):
        """Vérifier si la politique a changé significativement"""
        ...
        # On considère qu'une politique a changé si plus de 10% des actions de l'échantillon ont changé
        return changes > sample_size * 0.1

    def calculate_best_response(self, robot_id, other_policies):
        ...
        # Initialiser les valeurs avec une heuristique basée sur la distance
        V = self._initialize_values_with_heuristic(relevant_states)

        # Itération de valeur pour le MDP augmenté
        for iteration in range(self.max_iterations):
            ...
            if max_diff < self.epsilon:
                self.get_logger().info(f"VI converged after {iteration+1} iterations")
                break
    ...
    def _initialize_values_with_heuristic(self, states):
        """Initialiser les valeurs avec une heuristique basée sur la distance"""
        values = {}
    ...
    def publish_policies(self):
        ...
        self.get_logger().info("Published JESP policies")
